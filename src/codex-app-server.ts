...
export class CodexAppServerClient extends EventEmitter {
  ...
  async runTurn(threadId: string, text: string, turnTimeoutMs: number): Promise<TurnExecutionResult> {
    const preferred = this.preferredModel || undefined;
    const first = await this.executeTurn(threadId, text, preferred, turnTimeoutMs);
    ...
    if (
      ...
      const second = await this.executeTurn(threadId, text, this.fallbackModel, turnTimeoutMs);
